...
export default function PlaybackInterface({ tracks, token }) {
  ...
  return (
    <article className="absolute left-0 right-0 bottom-0 bg-slate-50/60 backdrop-blur-xs px-8 py-4 *:py-4 flex flex-wrap gap-x-6 justify-center items-center">
      <PlaybackControls {...playbackProps} isDisabled={false} />
      <PlaybackProgress {...playbackProps} />
    </article>
  );
}
...
function PlaybackProgress({
  ...
  const animationState = `${formatTrackProgress(duration_ms, progress)}%`;
  const props = {
  ...
  return (
    <>
      <article>
        <p className="text-sm">{formatDuration(progress)}</p>
      </article>
